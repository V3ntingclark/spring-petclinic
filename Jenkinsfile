pipeline {
  ...
  stages {
    stage('Checkout') {
      steps {
        withCredentials(bindings: [string(credentialsId: 'github-token', variable: 'GITHUB_TOKEN')]) {
          git(url: "https://${GITHUB_TOKEN}@github.com/V3ntingclark/spring-petclinic.git", branch: 'main', credentialsId: 'Jenkins')
          sh '''withCredentials([string(credentialsId: \'github-token\', variable: \'GITHUB_TOKEN\')]) {
